...
func (bfx *Bitfinex) GetKlineRecords(currencyPair CurrencyPair, period, size, since int) ([]Kline, error) {
	...
	if since!=0{
		log.Println("Bitfinex GetKlineRecords since is not been used")
	}
	var _period string
	switch period {
	...
	klineInterfaces, err := HttpGet3(bfx.httpClient, "https://api.bitfinex.com/v2/candles/trade:"+_period+":t"+strings.ToUpper(currencyPair.ToSymbol(""))+"/hist?limit="+strconv.Itoa(size), nil)
	...
	for _,klineInterface := range klineInterfaces{
		klineListInterface := klineInterface.([]interface{})

		kline := Kline{}

		kline.Symbol = currencyPair.ToSymbol("_")

		kline.Period = _period
		kline.Timestamp = int64(klineListInterface[0].(float64))
		kline.Open = klineListInterface[1].(float64)
		kline.Close = klineListInterface[2].(float64)
		kline.High = klineListInterface[3].(float64)
		kline.Low = klineListInterface[4].(float64)
		kline.Vol = klineListInterface[4].(float64)
		klines = append(klines,kline)
	}
	return klines,err
}

func (bfx *Bitfinex) GetSymbols() ([]CurrencyPair, error) {
	symbolInterfaces, err := HttpGet3(bfx.httpClient, "https://api.bitfinex.com/v1/symbols", nil)
	if err != nil {
		log.Println(err)
		return nil, err
	}

	symbolsList := []CurrencyPair{}
	for _, symbolInterface := range symbolInterfaces {
		symbol := symbolInterface.(string)
		switch {
		case strings.HasSuffix(symbol, "btc"):
			{
				symbolsList = append(symbolsList, NewCurrencyPair2(strings.Split(symbol, "btc")[0]+"_btc"))
			}
		case strings.HasSuffix(symbol, "eth"):
			{
				symbolsList = append(symbolsList, NewCurrencyPair2(strings.Split(symbol, "eth")[0]+"_eth"))
			}
		case strings.HasSuffix(symbol, "jpy"):
			{
				symbolsList = append(symbolsList, NewCurrencyPair2(strings.Split(symbol, "jpy")[0]+"_jpy"))
			}
		case strings.HasSuffix(symbol, "gbp"):
			{
				symbolsList = append(symbolsList, NewCurrencyPair2(strings.Split(symbol, "gbp")[0]+"_gbp"))
			}
		case strings.HasSuffix(symbol, "eur"):
			{
				symbolsList = append(symbolsList, NewCurrencyPair2(strings.Split(symbol, "eur")[0]+"_eur"))
			}
		case strings.HasSuffix(symbol, "usd"):
			{
				symbolsList = append(symbolsList, NewCurrencyPair2(strings.Split(symbol, "usd")[0]+"_usd"))
			}
		default:
			tool.NewMail("bitfinex有新symbol", "请检查")
			panic("New Symbol,please check")
		}
	}
